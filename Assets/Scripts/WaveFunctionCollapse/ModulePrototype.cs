	...
	static void DrawGizmo(ModulePrototype modulePrototype, GizmoType gizmoType)
	{
			...
			for (int i = 0; i < 6; i++)
			{
				...
				if (hint.Mesh != null)
				{
					Gizmos.DrawMesh(hint.Mesh,
						position + rotation * ToVector3(Orientations.Direction[i]) * BLOCK_SIZE,
		...
		for (int i = 0; i < 6; i++)
		{
			if (modulePrototype.Faces[i].Walkable)
			{
				...
				Gizmos.DrawLine(position + Vector3.down * 0.1f, position + rotation * Orientations.Rotations[i] * Vector3.forward * BLOCK_SIZE * 0.5f + Vector3.down * 0.1f);
			}
			if (modulePrototype.Faces[i].IsOcclusionPortal)
			{
				...
				Gizmos.DrawWireCube(position + dir, (Vector3.one - new Vector3(Mathf.Abs(dir.x), Mathf.Abs(dir.y), Mathf.Abs(dir.z))) * BLOCK_SIZE);
			}
		}
		...
		for (int i = 0; i < 6; i++)
		{
			...
			Handles.Label(position + rotation * Orientations.Rotations[i] * Vector3.forward * BLOCK_SIZE / 2f, face.ToString(), ModulePrototype.style);
		}
	}
#endif
